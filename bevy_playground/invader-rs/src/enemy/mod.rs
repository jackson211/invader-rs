...
use crate::{
    EnemyCount, GameTextures, WinSize, ENEMY_LASER_SIZE, ENEMY_MAX, SPRITE_SCALE, TIME_STEP,
};
    ...
    for &tf in enemy_query.iter() {
        commands
            .spawn_bundle(SpriteBundle {
                ...
                transform: Transform {
                    translation: Vec3::new(tf.translation.x, tf.translation.y - 15., 0.),
...
fn enemy_movement_system(
    ...
    for (mut transform, mut formation) in query.iter_mut() {
        ...
        // fixtures
        let dir = if formation.start.0 < 0. { 1. } else { -1. }; // -1 for counter clockwise, -1 clockwise
